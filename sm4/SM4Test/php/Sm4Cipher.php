...
class Sm4Cipher
{
    ...
    private const SM4_DECRYPT = 0;
    private const SM4_ENCRYPT = 1;
    ...
    public function enDataCbc($str, $iv)
    {
        $r = '';
        $this->ck16($iv);
        $str = $this->dd($str, static::SM4_ENCRYPT);
    ...
    public function deDataCbc($str, $iv)
    {
        ...
        return $this->dd($r, static::SM4_DECRYPT);
    }
    ...
    public function deDataEcb($str)
    {
        ...
        return $this->dd(pack('N*', ...$r), static::SM4_DECRYPT);
    }

    private function dd($data, $type)
    {
        if ($type == static::SM4_ENCRYPT) {
            $n = $this->b = $this->len - strlen($data) % $this->len;
            return $data . str_repeat(pack('C', $this->b), $n);
        }
        else if ($type == static::SM4_DECRYPT) {
            $n = unpack("C1", $data[strlen($data)-1])[1];
            return substr($data, 0, strlen($data) - $n);
        }
    }
    ...
    {
        return $this->box[($n & 0xff)] | $this->box[(($n >> 8) & 0xff)] << 8 | $this->box[(($n >> 16) & 0xff)] << 16 | $this->box[(($n >> 24) & 0xff)] << 24;
    }
